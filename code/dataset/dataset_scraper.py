...
parser.add_argument(
    "--api_key", type=str, default=DEFAULT_WINDY_API_KEY, help="Windy API Key"
)
parser.add_argument(
    "--dataset_path",
    type=str,
...
def get_webcam_images(webcam_id: int, timeframe: str) -> List[str]:
    embed_webcam_link = "{}/{}/{}".format(
        EMBED_WINDY_WEBCAM_URL, webcam_id, timeframe
    )
...
def download_images(image_url_list: List[str], dst_dir: str) -> None:
    if os.path.exists(dst_dir) and len(os.listdir(dst_dir)) > 5:
        logging.info(
            f"Stream dir {dst_dir} already exists and "
...
def main() -> None:
    ...
    for webcam in webcams:
        ...
        logging.info(
            "Downloading images from stream with id {}".format(webcam["id"])
        )

        download_images(
            webcam_images, os.path.join(args.dataset_path, webcam["id"])
        )
